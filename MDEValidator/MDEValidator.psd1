@{
    ...
    Copyright = '(c) 2024-2026 MDEValidator Team. All rights reserved.'
    ...
    PrivateData = @{
        PSData = @{
            ...
            ReleaseNotes = @'
2.0.0 - Major update:
- Performance: shared Get-MpPreference/Get-MpComputerStatus instances across all tests
- Code structure: split monolithic psm1 into categorised dot-sourced files
- Verbose/Debug: Write-Verbose and Write-Debug instrumentation throughout
- WPF UI: new Show-MDEValidatorUI function for graphical interface (Windows Desktop only)
- CI/CD: GitHub Actions pipeline for linting, testing, and PSGallery publishing
- Unit tests: Pester mocks for cross-environment test execution
'@
        }
    }
}
