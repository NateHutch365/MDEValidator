function Test-MDEOnboardingStatus {
    ...
    if ($null -eq $senseService) {
        Write-ValidationResult -TestName 'MDE Onboarding Status' -Category 'Onboarding' -Expected 'Onboarded' -Actual 'Not installed' -Status 'Fail' `
        ...
        Write-ValidationResult -TestName 'MDE Onboarding Status' -Category 'Onboarding' -Expected 'Onboarded' -Actual "$($senseService.Status)" -Status 'Fail' `
        ...
        Write-ValidationResult -TestName 'MDE Onboarding Status' -Category 'Onboarding' -Expected 'Onboarded' -Actual 'Running' -Status 'Pass' `
    ...
    if (Test-Path $onboardingPath) {
        ...
        if ($onboardingState.OnboardingState -eq 1) {
            Write-ValidationResult -TestName 'MDE Onboarding Registry State' -Category 'Onboarding' -Expected 'OnboardingState = 1' -Actual 'OnboardingState = 1' -Status 'Pass' `
                -Message 'Device is successfully onboarded to Microsoft Defender for Endpoint.'
        } else {
            Write-ValidationResult -TestName 'MDE Onboarding Registry State' -Category 'Onboarding' -Expected 'OnboardingState = 1' -Actual "OnboardingState = $($onboardingState.OnboardingState)" -Status 'Warning' `
        ...
        Write-ValidationResult -TestName 'MDE Onboarding Registry State' -Category 'Onboarding' -Expected 'OnboardingState = 1' -Status 'Warning' `
    ...
    if ($null -eq $diagTrackService -or $diagTrackService.Status -ne 'Running') {
        Write-ValidationResult -TestName 'Connected User Experiences and Telemetry Service' -Category 'Onboarding' -Expected 'Running' -Actual 'Not running' -Status 'Warning' `
        ...
        Write-ValidationResult -TestName 'Connected User Experiences and Telemetry Service' -Category 'Onboarding' -Expected 'Running' -Actual 'Running' -Status 'Pass' `
    ...
    if (Test-Path $onboardingPath) {
        ...
        if ($null -ne $orgIdProp -and $orgIdProp.OrgId) {
            Write-ValidationResult -TestName 'MDE Organization ID' -Category 'Onboarding' -Expected 'Configured' -Actual 'Configured' -Status 'Info' `
                -Message "OrgId is registered: $($orgIdProp.OrgId)"
        } else {
            Write-ValidationResult -TestName 'MDE Organization ID' -Category 'Onboarding' -Expected 'Configured' -Actual 'Not present' -Status 'Warning' `
        ...
        Write-ValidationResult -TestName 'MDE Organization ID' -Category 'Onboarding' -Expected 'Configured' -Status 'Warning' `
    ...
    try {
        ...
        Write-ValidationResult -TestName 'WDATPOnboarding Event Log' -Category 'Onboarding' -Expected 'Healthy' -Actual 'Healthy' -Status 'Pass' `
        ...
        Write-ValidationResult -TestName 'WDATPOnboarding Event Log' -Category 'Onboarding' -Expected 'Healthy' -Status 'Warning' `
