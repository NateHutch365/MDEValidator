function Test-MDEAutoExclusionsWindowsServer {
    ...
    if (-not (Test-IsWindowsServer)) {
        Write-ValidationResult -TestName $testName -Category 'Exclusion Settings' -Expected 'Enabled (auto)' -Status 'NotApplicable' `
    ...
    try {
        ...
        if ($mpPreference.DisableAutoExclusions -eq $true) {
            Write-ValidationResult -TestName $testName -Category 'Exclusion Settings' -Expected 'Enabled (auto)' -Actual 'Disabled' -Status 'Pass' `
                -Message "Auto Exclusions for Servers is disabled. Server roles will not automatically add exclusions."
        } else {
            Write-ValidationResult -TestName $testName -Category 'Exclusion Settings' -Expected 'Enabled (auto)' -Actual 'Enabled' -Status 'Warning' `
        ...
        Write-ValidationResult -TestName $testName -Category 'Exclusion Settings' -Expected 'Enabled (auto)' -Status 'Fail' `
